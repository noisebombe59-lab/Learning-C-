Access Modifiers (public/private/etc.) — «Когда я хочу выстроить границы: спрятать внутреннюю "кухню" класса и оставить снаружи только "кнопки управления"».
Static — «Когда мне нужен инструмент или переменная, которые общие для всех и не зависят от конкретного объекта (например, счетчик всех созданных пользователей или математическая формула)».
