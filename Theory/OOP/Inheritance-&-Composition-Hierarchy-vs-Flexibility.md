Inheritance (Наследование) — «Когда объекты имеют жесткую связь "является чем-то" (Кот — это Животное) и разделяют общую базу кода».
Composition (Композиция) — «Когда объект "состоит из" других частей; это гибче наследования, так как части можно менять на лету (у Робота можно заменить руку-манипулятор на руку-лазер)».
