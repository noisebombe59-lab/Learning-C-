    1. API (Интерфейс прикладного программирования)Это целостный продукт. Твой бэкенд-проект в Visual Studio — это приложение, а та его часть, которую ты выставил наружу для общения с фронтендом, называется API.Суть: Это набор правил и «договор» между тобой и другими разработчиками.Из чего состоит: Из всех твоих контроллеров, эндпоинтов, моделей данных (DTO) и правил безопасности.Роль: Позволить фронтенду запустить твой сложный код на сервере через простые команды.
    2. Маршрутизация (Routing)Это навигационная система внутри твоего API. Она не является «кодом» или «адресом», это механизм, который сопоставляет входящий запрос с твоим кодом.Суть: Логика поиска. Когда прилетает запрос, маршрутизация бежит по списку твоих атрибутов [Route] и ищет совпадение.
