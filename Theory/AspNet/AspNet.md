    ...
    Маршрутизация (Routing)Это навигационная система внутри твоего API. Она не является «кодом» или «адресом», это механизм, который сопоставляет входящий запрос с твоим кодом.Суть: Логика поиска. Когда прилетает запрос, маршрутизация бежит по списку твоих атрибутов [Route] и ищет совпадение.
    Endpoint (Конечная точка)Это конкретная точка встречи адреса и действия. Это не просто строчка кода, это «рабочий контакт».Формула Эндпоинта:$$\text{Endpoint} = \text{URL-путь (маршрут)} + \text{HTTP-метод (действие)}$$Эндпоинт 1: GET + /api/stock $\rightarrow$ Метод GetAll()Эндпоинт 2: POST + /api/stock $\rightarrow$ Метод Create()Эндпоинт 3: GET + /api/stock/5 $\rightarrow$ Метод GetById(5)
