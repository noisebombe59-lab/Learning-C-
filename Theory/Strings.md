...
`Dictionary<TKey, TValue>` — коллекция пар «ключ → значение». Главное преимущество: поиск по ключу почти мгновенно (O(1) в среднем). Ключи всегда уникальны (дубли не добавляются). Под капотом — хеш-таблица.
