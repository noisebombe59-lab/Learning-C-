...
* **Class vs Struct vs Record** (Зачем в C# добавили `record`, их иммутабельность, сравнение по значению и сценарии применения).
...
#### Делегаты и События (Delegates & Events)
* **Встроенные делегаты:** `Func<T>`, `Action<T>`, `Predicate<T>`. Понимание того, как они работают под капотом лямбда-выражений в LINQ.
...
* **Обработка ошибок:** Использование конструкции `try-catch-finally`.
* **Срез стека ошибок:** Принципиальное отличие `throw;` от `throw ex;` (сохранение оригинального StackTrace против его перезаписи).
...
### Как изучать и проверять себя
Напишите консольное приложение, которое читает большой текстовый файл, парсит его, группирует данные через LINQ и сохраняет в `Dictionary`. Измерьте время работы. Проверьте через профилировщик памяти (хотя бы встроенный в Visual Studio), сколько памяти выделилось.
...
* **Конфигурация (Configuration):** Работа с файлом `appsettings.json` через паттерн `IOptions<T>` (безопасный вынос настроек, секретов и строк подключения к БД из кода).
...
* **Логирование (Logging):** Использование встроенного интерфейса `ILogger` (`_logger.LogInformation`, `_logger.LogError`) в критических точках приложения. Общее представление о Serilog (структурированное логирование).
...
### Как изучать и проверять себя
Создайте API для гипотетического магазина. Напишите Middleware, который логирует время выполнения каждого HTTP-запроса. Настройте JWT-аутентификацию так, чтобы создавать товары мог только "Admin", а просматривать — кто угодно.
...
### Как изучать и проверять себя
Напишите два одинаковых запроса: один через EF Core, а второй — чистым SQL через консоль PostgreSQL (pgAdmin / DBeaver). Включите логирование EF Core в `appsettings.json`, чтобы видеть реальный SQL, который генерирует ORM. Добейтесь того, чтобы запрос не делал лишних `JOIN`.
...
### Как изучать и проверять себя
Выделите расчет стоимости заказа в отдельный сервис (класс). Напишите для него 5 Unit-тестов, проверяющих разные краевые условия (пустая корзина, применение скидки, отрицательное количество товаров).

---

## Фаза 5: Асинхронность и Воркеры (Async & Background Tasks)
...
* **Инструменты автоматизации:** Работа с `BackgroundService` и `IHostedService` (создание простых периодических задач внутри приложения, например, очистка кэша раз в час).
...
### Как изучать и проверять себя
Сделайте эндпоинт `/submit-order`. Вместо того чтобы считать тяжелую аналитику прямо во время HTTP-запроса, отправьте сообщение в RabbitMQ через MassTransit. Напишите Consumer, который поймает это сообщение в фоновом режиме и обработает его.
